  ...
  describe('Push ' + id, function () {
    ...
    describe('Single', function () {
      ...
      it('With header', function () {
        var buf = 'abcdef'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'single']])
        .then(Fn.shouldHaveWritten(1))
      })
      ...
      it('With bad header', function () {
        var buf = 'abcdef'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'invalid header']])
    ...
    describe('Atomic', function () {
      it('Without separator', function () {
        ...
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'atomic']])
        .then(Fn.shouldHaveWritten(1))
      })
    ...
    describe('Custom IDs', function () {
      it('Without separator, single mode', function () {
        var buf = 'A abc\nA def\nA ghi\nA jkl'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'single'], [C.idHeader, 'id1']])
        .then(Fn.shouldHaveWritten(1))
      })

      it('With separator, single mode', function () {
        var buf = 'A abc\nA def\nA ghi\nA jkl'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'single'], [C.idHeader, 'id1,id2']])
        .then(Fn.shouldHaveWritten(1))
      })

      it('Without separator', function () {
        var buf = 'abcdef'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'single'], [C.idHeader, 'id10,id11,id12,id13']])
        .then(Fn.shouldHaveWritten(1))
      })

      it('With separator', function () {
        var buf = 'A abc\nA def\nA ghi\nA jkl'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'multiple'], [C.idHeader, 'id10,id11,id12,id13']])
        .then(Fn.shouldHaveWritten(4))
      })

      it('With separator, non-matching lengths 1', function () {
        var buf = 'A abc\nA def\nA ghi\nA jkl'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'multiple'], [C.idHeader, 'id20,id21,id22']])
        .then(Fn.shouldFail(400))
      })

      it('With separator, non-matching lengths 2', function () {
        var buf = 'A abc\nA def\nA ghi'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'multiple'], [C.idHeader, 'id30,id31,id32,id33']])
        .then(Fn.shouldFail(400))
      })

      it('With separator, empty IDs 1', function () {
        var buf = 'A abc\nA def\nA ghi'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'multiple'], [C.idHeader, 'id40,id41,id42,']])
        .then(Fn.shouldFail(400))
      })

      it('With separator, empty IDs 2', function () {
        var buf = 'A abc\nA def\nA ghi'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'multiple'], [C.idHeader, 'id50,id51,,id52,id53']])
        .then(Fn.shouldFail(400))
      })

      it('With separator, atomic', function () {
        var buf = 'A abc\nA def\nA ghi'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'atomic'], [C.idHeader, 'id60,id61,id62,']])
        .then(Fn.shouldHaveWritten(1))
      })

      it('With separator, skip existing', function () {
        var buf = 'A abc\nA def\nA ghi'
        return Fn.call('POST', 8000, '/example', buf, [[C.modeHeader, 'multiple'], [C.idHeader, 'id70,id70,id1']])
        .then(Fn.shouldHaveWritten(1))
      })
    })

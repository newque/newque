...
let create base_urls ~index ~typename =
  ...
  let instance = { base_urls; index; typename; } in
  return instance
  ...
  let push instance ~msgs ~ids =
    ...
    | true ->
      ...
      wrap (fun () ->
        ...
        let (strings: string list) = List.filter_map items ~f:(fun item ->
    ...
    try%lwt
      ...
      begin match Code.code_of_status (Response.status response) with
        ...
        | code ->
          return [
            Printf.sprintf
              "Couldn't validate index [%s] at %s. Received: %s (%d)"
              instance.index (Uri.to_string uri) (Code.string_of_status (Response.status response)) code
          ]
      end
    with
